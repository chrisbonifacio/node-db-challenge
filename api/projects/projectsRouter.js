...
router.get("/", async (req, res) => {
  try {
    ...
    // coerce completed value from 0-1 to a Boolean
    projects = projects.map(project => {
      return { ...project, completed: Boolean(project.completed) }
    })
    res.status(200).json(projects)
  } catch (error) {
    res.status(400).json({ erorMessage: "Could not retrieve projects" })
  }
})
...
router.get("/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(400)
      .json({ errorMessage: "Could not get Project with specified ID" })
  }
})
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ ...newProject, completed: Boolean(newProject.completed) })
  } catch (error) {
    res.status(400).json({ errorMessage: "Could not add project to database" })
  }
})
...
router.post("/tasks", async (req, res) => {
  ...
  try {
    ...
    res.status(400).json({ errorMessage: "Could not add task to project" })
  }
})
